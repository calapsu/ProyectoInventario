...
var URLState = function (url, isBase, base) {
  ...
  if (isBase) {
    ...
    if (failure) throw TypeError$i(failure);
...
var notARegexp = function (it) {
  if (isRegexp(it)) {
    throw TypeError$j("The method doesn't accept regular expressions");
  } return it;
};
  ...
  concat: function concat(arg) {
    ...
    for (i = -1, length = arguments.length; i < length; i++) {
      ...
      if (isConcatSpreadable(E)) {
        ...
        if (n + len > MAX_SAFE_INTEGER) throw TypeError$k(MAXIMUM_ALLOWED_INDEX_EXCEEDED);
        for (k = 0; k < len; k++, n++) if (k in E) createProperty(A, n, E[k]);
      } else {
        if (n >= MAX_SAFE_INTEGER) throw TypeError$k(MAXIMUM_ALLOWED_INDEX_EXCEEDED);
