...
var getIterator = function (argument, usingIterator) {
  ...
  throw TypeError$e(tryToString(argument) + ' is not iterable');
};
  ...
  parseObject: function (object) {
    ...
    if (iteratorMethod) {
      ...
      while (!(step = functionCall(next, iterator)).done) {
        ...
        ) throw TypeError$g('Expected sequence with length 2');
        push$6(this.entries, { key: toString_1(first.value), value: toString_1(second.value) });
      }
    } else for (var key in object) if (hasOwnProperty_1(object, key)) {
      push$6(this.entries, { key: key, value: toString_1(object[key]) });
    }
  },
...
var URLState = function (url, isBase, base) {
  ...
  if (isBase) {
    ...
    if (failure) throw TypeError$h(failure);
